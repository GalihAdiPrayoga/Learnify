...
const MaterialPage = () => {
  ...
  // Find the first incomplete unlocked material (current active)
  const firstIncompleteIndex = materials.findIndex((m) => {
    const isCompleted = localCompleted.has(m.id);
      ...
      {materials.length === 0 ? (
        ...
        <div className="space-y-4">
          {materials.map((material, index) => {
            ...
            const isLocked = material.is_locked === true;
            const isCurrentActive =
              !isLocked && !isCompleted && index === firstIncompleteIndex;
            return (
              <MaterialItem
                key={material.id}
                material={material}
